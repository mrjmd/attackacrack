...
class TestRecipientAssignment:
    """Test recipient assignment to variants"""
    
    def test_assign_recipients_50_50_split(self, service, sample_campaign, sample_contacts, mock_ab_result_repo):
        ...
        # With 4 contacts and 50/50 split, should be 2 each
        assert variant_a_count == 2
        assert variant_b_count == 2
        assert len(assignments) == 4
    
    def test_assign_recipients_70_30_split(self, service, sample_campaign, sample_contacts, mock_ab_result_repo):
        ...
        # With 10 contacts and 70/30 split, should be 7 A and 3 B
        assert variant_a_count == 7
        assert variant_b_count == 3
    
    def test_assignment_is_deterministic(self, service, sample_campaign, sample_contacts, mock_ab_result_repo):
    ...
    def test_track_variant_assignment(self, service, sample_campaign, sample_contacts, mock_ab_result_repo):
...
class TestStatisticalSignificance:
    ...
    def test_insufficient_sample_size(self, service):
        ...
        assert result.is_success
        significance_data = result.data
        assert significance_data['significant'] is False
...
class TestWinnerSelection:
    ...
    def test_identify_automatic_winner(self, service, mock_ab_result_repo, mock_campaign_repo):
        ...
        campaign_id = 1
        ab_summary = {
            'variant_a': {'conversion_rate': 0.12, 'statistical_confidence': 0.85},
            'variant_b': {'conversion_rate': 0.18, 'statistical_confidence': 0.96},
            'winner': 'B',
        ...
        mock_campaign_repo.update_ab_winner.return_value = Result.success(True)
        ...
        mock_campaign_repo.update_ab_winner.assert_called_once_with(
            campaign_id, 'B', winner_data
        )
    ...
    def test_no_winner_below_confidence_threshold(self, service, mock_ab_result_repo):
        ...
        ab_summary = {
            'variant_a': {'conversion_rate': 0.12, 'statistical_confidence': 0.85},
            'variant_b': {'conversion_rate': 0.15, 'statistical_confidence': 0.88},
            'winner': 'B',
    ...
    def test_manual_winner_override(self, service, mock_campaign_repo):
        ...
        mock_campaign_repo.update_ab_winner.assert_called_once()
    ...
    def test_send_winner_to_remaining_recipients(self, service, mock_campaign_repo, mock_contact_repo):
        ...
        mock_campaign_repo.get_remaining_recipients.return_value = Result.success(remaining_contacts)
        mock_campaign_repo.get_campaign.return_value = Result.success(
            Campaign(id=campaign_id, template_b="Winning message template")
        )
...
class TestABTestReporting:
    ...
    def test_generate_ab_test_report(self, service, mock_ab_result_repo, mock_campaign_repo):
        ...
        mock_campaign_repo.get_campaign.return_value = Result.success(campaign)
        mock_ab_result_repo.get_campaign_ab_summary.return_value = Result.success(ab_summary)
    ...
    def test_generate_report_for_ongoing_test(self, service, mock_ab_result_repo, mock_campaign_repo):
        ...
        ab_summary = {
            'variant_a': {'conversion_rate': 0.10, 'messages_sent': 50},
            'variant_b': {'conversion_rate': 0.12, 'messages_sent': 50},
            'winner': None,
        ...
        assert 'continue_test' in report['recommendations'][0].lower()
